"""
Crop Classification Project - Step 3: Apply Model to Full Satellite Image
==========================================================================
Classifies the full Sentinel-2 GeoTIFF using trained models.
Uses GDAL for GeoTIFF I/O, preserving CRS and geotransform.
Handles band order mismatch between CSV training data and GeoTIFF.
"""
...
print("=" * 70)
print("CROP CLASSIFICATION - APPLY MODEL TO SATELLITE IMAGE")
print("=" * 70)
...
models = {}
model_files = {
    'MLP': 'crop_classifier_mlp.joblib',
    'MLP_SMOTE': 'crop_classifier_mlp_smote.joblib',
    'XGBoost': 'crop_classifier_xgb.joblib',
    'XGB_SMOTE': 'crop_classifier_xgb_smote.joblib',
    'RF': 'crop_classifier_rf.joblib',
}
...
print(f"\n    Selected model for prediction: {SELECTED_MODEL}")
...
for b in range(1, n_bands + 1):
    ...
    if desc:
        tif_band_names[desc] = b  # band name -> 1-based band index

print(f"\n    GeoTIFF band names: {list(tif_band_names.keys())}")

# Build reorder mapping: for each CSV feature, find the matching GeoTIFF band
# This is the critical fix - ensures bands are read in the correct order
band_read_order = []  # list of 1-based GeoTIFF band indices, in CSV feature order
matched = 0
...
if matched != len(feature_cols):
    ...
    # Fill missing with band index (fallback)
    for i in range(len(band_read_order)):
        if band_read_order[i] is None:
            band_read_order[i] = i + 1

# Verify the reordering is not identity (same order)
...
# ============================================================================
# 3. CLASSIFY IMAGE IN CHUNKS (with correct band order)
# ============================================================================
...
for chunk_idx in range(n_chunks):
    ...
    chunk_2d = chunk_data.reshape(n_features, -1).T  # (pixels, features)
    ...
    if n_valid > 0:
        ...
        predictions = model.predict(chunk_scaled)
...
output_path = OUTPUT_DIR / 'classification_map.tif'
...
# ============================================================================
# 6. SAVE RGB CLASSIFICATION MAP
# ============================================================================
print("\n[6] Saving RGB classification map...")

rgb_path = OUTPUT_DIR / 'classification_map_rgb.tif'
...
rgb_ds = driver.Create(str(rgb_path), width, height, 3, gdal.GDT_Byte,
                       options=['COMPRESS=LZW', 'TILED=YES'])
rgb_ds.SetGeoTransform(geotransform)
rgb_ds.SetProjection(projection)
for band_idx in range(3):
    band = rgb_ds.GetRasterBand(band_idx + 1)
    band.WriteArray(rgb[:, :, band_idx])
    band.FlushCache()
rgb_ds = None
...
# ============================================================================
print("\n[7] Creating classification visualization...")
...
axes[0].set_title(f'Crop Classification Map\n{SELECTED_MODEL} Model | Elgabel Region, Sudan',
...
# Filter out zero-area classes for pie chart
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / 'classification_visualization.png', dpi=200, bbox_inches='tight')
plt.show()
print(f"    Saved: {OUTPUT_DIR / 'classification_visualization.png'}")

# ============================================================================
# 8. TRUE COLOR vs CLASSIFICATION COMPARISON
# ============================================================================
print("\n[8] Creating true color vs classification comparison...")

try:
    # Read RGB bands using band descriptions
    ...
    if b4_band and b3_band and b2_band:
        ...
        axes[1].set_title(f'Classification Map ({SELECTED_MODEL})', fontsize=13, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(OUTPUT_DIR / 'truecolor_vs_classification.png', dpi=200, bbox_inches='tight')
        plt.show()
        print(f"    Saved: {OUTPUT_DIR / 'truecolor_vs_classification.png'}")
    else:
...
# ============================================================================
# 9. CLASSIFY WITH ALL MODELS (comparison)
# ============================================================================
print("\n[9] Comparing all models on full image...")

# Reload and read bands in correct order
ds = gdal.Open(str(IMAGE_PATH), gdal.GA_ReadOnly)

full_data = np.zeros((n_features, height, width), dtype=np.float32)
for feat_idx, tif_band in enumerate(band_read_order):
    full_data[feat_idx] = ds.GetRasterBand(tif_band).ReadAsArray().astype(np.float32)
ds = None

pixels_2d = full_data.reshape(n_features, -1).T
valid_mask = np.all(np.isfinite(pixels_2d), axis=1) & np.any(pixels_2d != 0, axis=1)
pixels_scaled = scaler.transform(pixels_2d[valid_mask])
...
for name, mdl in models.items():
    t0 = time.time()
    preds = mdl.predict(pixels_scaled)
    t1 = time.time()
    model_predictions[name] = preds

    # Class distribution for this model
    unique, counts = np.unique(preds, return_counts=True)
    dist = {CLASS_NAMES[int(u)]: c for u, c in zip(unique, counts)}
    print(f"    {name:<12}: {t1-t0:.2f}s | {dist}")

# Model agreement
pred_array = np.stack(list(model_predictions.values()), axis=0)
agreement = np.all(pred_array == pred_array[0], axis=0)
print(f"\n    All models agree: {agreement.sum():,} / {len(agreement):,} ({agreement.mean()*100:.2f}%)")
...
print(f"""
    Output Files:
    -------------
    - outputs/classification_map.tif           (GeoTIFF, opens in QGIS with colors)
    - outputs/classification_map_rgb.tif       (3-band RGB GeoTIFF)
    - outputs/classification_visualization.png
    - outputs/truecolor_vs_classification.png
""")
